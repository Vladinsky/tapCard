...
export function BusinessLandingPage({ business, demo = false }: { business: Business; demo?: boolean }) {
      ...
      <div className="business-shell">
        <article className="business-card">
          ...
          <div className="business-content">
            {review ? <ReviewCTA href={review} /> : <p className="business-notice">{actions.length ? 'Le recensioni non sono al momento disponibili' : 'I collegamenti di questa attività non sono al momento disponibili'}</p>}
            {actions.length > 0 && <nav className="business-actions" aria-label="Collegamenti dell’attività">{actions.map((action) => <ActionCard key={action.id} action={action} />)}</nav>}
